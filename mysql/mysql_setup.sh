...
mysql_6_version=5.6.22
dbrootpwd=root
SETUP_FILE_NAME="mysql-${mysql_6_version}.tar.gz"
SETUP_DIR_NAME="mysql-${mysql_6_version}"
...
Install_MySQL-5-6() {
    ...
    if [ ! -f ${SETUP_FILE_NAME} ]; then
        ...
        echo "${SETUP_FILE_NAME} on local disk...."
        if [ -d ${SETUP_DIR_NAME} ]; then
            echo "remove old dir...."
            rm -rf ${SETUP_DIR_NAME}
        fi
    fi
    ...
    if [ -d "$mysql_install_dir/support-files" ]; then
        ...
        if [ ! -f "/usr/bin/mysql" ]; then
            echo "make ln for mysql.."
            $(ln -s $mysql_install_dir/bin/mysql /usr/bin/mysql)
        fi

        echo "dbrootpwd: ${dbrootpwd}"
